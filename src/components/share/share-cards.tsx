...
export const SHARE_CARDS: { id: ShareCardId; label: string; caption: string }[] = [
  { id: "receipt", label: "The receipt", caption: "Your three biggest line items." },
  { id: "statement", label: "The excuse", caption: "For people who say they're busy." },
];
...
export function ReceiptShareCard({
  ...
  const punchline = buildPunchline(result);
  const who = (name ?? "").trim();
      ...
      <dl className="relative z-2 flex gap-[48px] border-t-2 border-ink pt-[18px]">
        <div>
          ...
          <dd className="tnum mt-[2px] font-display text-[38px] leading-none">
            {numbers.format(result.daysLived)}
          </dd>
        </div>
        <div>
          <dt className="font-mono text-[12px] tracking-[0.2em] text-ink-faint uppercase">
          ...
          <dd className="tnum mt-[2px] font-display text-[38px] leading-none">
            {numbers.format(result.daysRemaining)}
          </dd>
        </div>
      </dl>
 ...
 */
function heroFigure(years: number) {
  if (years < 1) {
    const months = Math.max(1, Math.round(years * 12));
    return { whole: String(months), fraction: null, unit: months === 1 ? "month" : "months" };
  }
...
export function StatementShareCard({ result }: { result: LifeResult }) {
  ...
  const label = hero ? heroLabel(hero) : "Being busy";
  const { whole, fraction, unit } = heroFigure(years);
    ...
    >
      ...
      <div className="mt-[14px] border-t-2 border-b-2 border-paper/35 py-[24px]">
        ...
        <p className="tnum mt-[4px] flex items-baseline font-display leading-[0.8]">
          ...
          {fraction ? <span className="text-[90px]">.{fraction}</span> : null}
        ...
        <dl className="mt-[26px] flex gap-[44px] font-mono">
          <div>
            ...
            <dd className="tnum mt-[4px] text-[30px] font-bold">
              {formatYearsDecimal(hero.yearsSpent)}
            </dd>
          </div>
          <div>
            <dt className="text-[13px] tracking-[0.2em] uppercase opacity-55">
            ...
            <dd className="tnum mt-[4px] text-[30px] font-bold">
              {formatYearsDecimal(hero.yearsRemaining)}
            </dd>
          </div>
        </dl>
      ) : null}
      ...
      <p className="mt-auto max-w-[22ch] pt-[28px] text-[24px] leading-[1.35] text-paper/75">
        Find out where the rest of yours is going.
      </p>
...
function heroLabel(item: LineItem) {
  switch (item.id) {
    case "social":
      return "Scrolling";
    case "streaming":
